...
function finity.new(TipName, Size, InstanceName, UIFunction)
	local finityObject = {}
	...
	function self2:SetName(Name) 
		gsub(self2.tip.Text, finity.TipName, Name) 
		finity.TipName = Name 
		self2.tip.Text = Name  
	end 
	...
	function self2:SetCategoryStorage(table) 
		for i,v in next, self2.Categories do 
			table[i] = v 
		end 
		for i,v in next, self2.Categories do 
			self2.Categories[i] = nil 
		end 
		self2.Categories = table 
	end 
	self2.Sectors = {} 
	function self2:SetSectorStorage(table) 
		for i,v in next, self2.Sectors do 
			table[i] = v 
		end 
		for i,v in next, self2.Sectors do 
			self2.Sectors[i] = nil 
	...
	function self2:SetCheatStorage(table)  
		for i,v in next, self2.Features do 
			table[i] = v 
		end 
		for i,v in next, self2.Features do 
			self2.Features[i] = nil 
		end 
		self2.Features = table 
	end 
	...
	function self2:Category(name)
		...
		function category:Sector(name)
			...
			function sector:Cheat(kind, name, callback, data)
				...
				local cheat = {}
				cheat.flags = data and data.flags
				cheat.func = callback
				cheat.value = nil
				cheat.override = data and data.kind == 2 and kind == 'button'
				...
				cheat.label = finity:Create(cheat.override and 'TextButton' or 'TextLabel', {
					...
					TextSize = 13,
					Text = name,
					TextTruncate = data and data.truncate,
					RichText = true,
					TextXAlignment = Enum.TextXAlignment.Left
				})
				...
				if kind then
					if kind == "checkbox" or kind == "toggle" then
						...
						function cheat:PauseAllCheats(state) 
							for i,v in pairs(finity.checkbox_cache) do 
								v:pause(state)
							end
						end
                    ...
                    elseif kind:find("color") then
						...
						cheat.hsvbar.Activated:Connect(function(self, clickCount)
                        ...
                        cheat.hsvbar.MouseButton1Down:Connect(function()
                            ...
                            local uis = UserInputService or game.UserInputService
					...
					elseif kind == "slider" then
						cheat.value = data and data.default or 0
